...
status_t IMAGE_GetImage(uint8_t* dstData, int32_t dstWidth, int32_t dstHeight, int32_t dstChannels)
{
    ...
    if (s_staticCount == 1)
    {
        ...
        return IMAGE_Decode(image_data_1, dstData, dstWidth, dstHeight, dstChannels);
    }
    else if (s_staticCount == 2)
    {
        PRINTF(EOL "Static data processing:" EOL);
        return IMAGE_Decode(image_data_5, dstData, dstWidth, dstHeight, dstChannels);
    }
    else if (s_staticCount == 3)
    {
        PRINTF(EOL "Static data processing:" EOL);
        return IMAGE_Decode(image_data_81, dstData, dstWidth, dstHeight, dstChannels);
    }
    else
    {
